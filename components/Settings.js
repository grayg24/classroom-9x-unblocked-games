...
const Settings = ({ user, onUpdateSettings, onSetTheme, onRedeemCode }) => {
  ...
  const Toggle = ({ active, onToggle, icon: Icon, title, description, disabled = false, lockedLevel = 0 }) => html`
    <div 
      ...
      onMouseEnter=${() => !disabled && window.setCursorActive && window.setCursorActive(true)}
      onMouseLeave=${() => !disabled && window.setCursorActive && window.setCursorActive(false)}
    >
      <div className="flex items-start gap-5">
        ...
        <div>
          <h3 className="font-orbitron font-bold text-white uppercase text-sm tracking-widest mb-1">
            ${title}
            ${disabled && html`<span className="ml-2 text-theme text-[10px] bg-theme/10 px-2 py-0.5 rounded">LVL ${lockedLevel}</span>`}
          </h3>
             ...
             <div className="p-8 bg-slate-900/60 rounded-[2.5rem] border border-white/5 relative overflow-hidden group">
                ...
                <form onSubmit=${handleRedeem} className="flex gap-2">
                   <input 
                    ...
                    className="flex-1 bg-black/40 border border-white/10 rounded-xl px-4 py-3 font-orbitron text-xs text-theme focus:outline-none transition-all placeholder:text-slate-700 terminal-input"
                   />
                   <button 
                    type="submit"
                    onMouseEnter=${() => window.setCursorActive?.(true)}
